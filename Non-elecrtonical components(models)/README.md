...
## **Steering and 4-Gear Mechanical Differential systems** 
The steering system is a hybrid LEGO and 3D-printed design that provides precise turning with minimal friction by using a simple front-wheel steering mechanism connected to a single servo, combined with a LEGO differential to ensure smooth and stable wheel rotation during turns, while all moving parts are carefully aligned and lightly lubricated to reduce friction and improve consistency, allowing the robot to execute accurate and repeatable movements on a WRO 3×3 meter field.
